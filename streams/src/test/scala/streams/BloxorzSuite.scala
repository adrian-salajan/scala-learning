...
class BloxorzSuite extends FunSuite {
  ...
  test("block is in terrain or not") {
    new Level1 {
      assert(Block(Pos(0, 7), Pos(1, 7)).isLegal == false)
      assert(Block(Pos(2, 2), Pos(3, 2)).isLegal)
    }
  }

  test("block is starting correct, upright") {
    new Level1 {
      assert(startBlock.isStanding && startBlock.b1 == Pos(1,1))
    }
  }
